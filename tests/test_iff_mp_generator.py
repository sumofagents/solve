...
def test_generator_emits_ordered_iff_candidates():
    candidates = generate_iff_mp_candidates(
        [atom("B.iff", "Q ↔ R"), atom("A.iff", "P ↔ Q"), atom("C.bad", "P → Q")],
        max_candidates=10,
        experiment_name="x",
    )
    assert [candidate.parents for candidate in candidates] == [("A.iff",), ("B.iff",)]
    assert [candidate.statement for candidate in candidates] == ["P → Q", "Q → R"]
    assert all(candidate.operator == "Iff.mp" for candidate in candidates)
...
def test_generator_zero_cap_yields_empty_list():
    assert generate_iff_mp_candidates([atom("A.iff", "P ↔ Q")], max_candidates=0, experiment_name="x") == []


def test_generator_is_deterministic_and_ids_are_stable():
    atoms = [atom("B.iff", "Q ↔ R"), atom("A.iff", "P ↔ Q")]
    first = generate_iff_mp_candidates(atoms, max_candidates=10, experiment_name="x")
    second = generate_iff_mp_candidates(atoms, max_candidates=10, experiment_name="x")
    assert first == second
